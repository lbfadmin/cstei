...
<head>
    <meta charset="utf-8" />
    <title>行业分布管理 - <?=$_TPL['site_name']?></title>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form form-horizontal" method="post">
                    ...
                    <div class="form-group">
                        <label class="control-label col-md-3">比重 <span class="">&nbsp;</span></label>
                        <div class="col-md-8">
...
<script>
    $(function () {
        ...
        $modalForm.find('.ok').on('click', function () {
            ...
            if (action == 'add') {
                $.ajax({
                    url: '/supervisor/ajax/industry-weight/add',
                    type: 'post',
            ...
            if (action === 'update') {
                $.ajax({
                    url: '/supervisor/ajax/industry-weight/update',
                    type: 'post',
                    data: $modalForm.find('form').serialize(),
                    success: function (result) {
                        $modalForm.modal('hide');
        ...
        $('[data-role=delete-item]').on('click', function () {
            ...
            layer.confirm('您确定要删除这个吗（如果比重下有子比重请先删除子比重）？', function () {
                $.ajax({
                    url: '/supervisor/ajax/industry-weight/delete',
                    type: 'post',
                    data: {id: item.id},
                    success: function (result) {
