                ...
                <div class="page-content">
                    ...
                    <div class="portlet light">
                        <div class="portlet-title">
                            ...
                            <div class="actions">
                                <a class="btn btn-circle btn-primary" href="<?=url('device/device/index') ?>">
                                    <i class="fa fa-plus"> 添加申请</i>
                                </a>
                            </div>
                        </div>
                                            ...
                                            <td>
                                                <div class="btn-group">
                                                    <?php if($item->status==0):?>
                                                        <a href="#" data-toggle="tooltip"  title="" class="btn btn-default btn-xs" >设备申请中</a>
                                                    <?php endif ?>
                                                    <?php if($item->status==1):?>
                                                        <a href="#" data-toggle="tooltip" data-role="start" data-id="<?=$item->id?>" title="" class="btn btn-default btn-xs" ><i class="fa fa-edit"></i> 开始使用</a>
                                                    <?php endif ?>
                                                    <?php if($item->status==2):?>
                                                        <a href="#" data-toggle="tooltip" title="" class="btn btn-default btn-xs" ><i class="fa fa-edit"></i> 已拒绝</a>
                                                    <?php endif ?>
                                                    <?php if($item->status==3):?>
                                                        <a href="javascript:" data-toggle="tooltip"  title="" class="btn btn-default btn-xs" data-role="back" data-id="<?=$item->id?>"><i class="fa fa-edit"></i> 归还</a>
                                                    <?php endif ?>
                                                    <?php if($item->status==4):?>
                                                        <a href="#" data-toggle="tooltip" title="" class="btn btn-default btn-xs" ><i class="fa fa-edit"></i> 已归还</a>
                                                    <?php endif ?>
    ...
    <script>
        $(function () {
            $('[data-role=start]').on('click', function () {
                ...
                var status=3;
                layer.confirm('您确定要使用这个设备吗？', function () {
                    $.ajax({
                        url: '/device/ajax/device-use/usestatus',
                        type: 'post',
            ...
            $('[data-role=back]').on('click', function () {
                var $this = $(this),
                    id = $this.attr('data-id');
                var status =4;
                layer.confirm('您确定要归还这个设备吗？', function () {
                    $.ajax({
                        url: '/device/ajax/device-use/usestatus',
                        type: 'post',
                        data: {id: id,status:status},
