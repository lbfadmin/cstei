...
<head>
    <meta charset="utf-8" />
    <title>资讯分类管理 - <?=$_TPL['site_name']?></title>
...
<script>
    $(function () {
        ...
        $modalForm.find('.ok').on('click', function () {
            ...
            if (action == 'add') {
                $.ajax({
                    url: '/content/ajax/info/category/add',
                    type: 'post',
            ...
            if (action === 'update') {
                $.ajax({
                    url: '/content/ajax/info/category/update',
                    type: 'post',
                    data: $modalForm.find('form').serialize(),
                    success: function (result) {
                        $modalForm.modal('hide');
        ...
        $('[data-role=delete-category]').on('click', function () {
            ...
            layer.confirm('您确定要删除这个分类吗（如果分类下有子分类请先删除子分类）？', function () {
                $.ajax({
                    url: '/content/ajax/info/category/delete',
                    type: 'post',
                    data: {id: category.id},
                    success: function (result) {
