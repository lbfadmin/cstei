...
<div class="mbxmenu center-block">
    <p>您的位置： <a href="">首页</a>>><a href="">成果展示</a><?php if($title != '成果展示'):?>>><a href=""><?=$title?></a><?php endif ?></p>
</div>
...
<?php foreach ($category->children as $item):?>
           <li class="">
                <a href="/content/news/achieve?category_id=<?=$item->id?>&name=<?=$item->name?>">
                    <h1><?=$item->name?></h1>
...
<?php if (!empty($result->list)):?>
        <div class="innercontentitem">
            <div class="innercontentitem9">
                ...
                <li class="listwz">
                    <a href="/content/news/achieve-detail?id=<?=$item->id?>">
                        <dd></dd>
                        <p><?=$item->title?> </p>
