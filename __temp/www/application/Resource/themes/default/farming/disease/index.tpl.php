...
<head>
    <meta charset="utf-8" />
    <title>病害信息 - <?=$_TPL['site_name']?></title>
                                            ...
                                            <div class="portlet light ">
                                                <div class="portlet-title">
                                                    <div class="caption caption-md">
                                                        ...
                                                        <span class="caption-subject font-green-steel uppercase bold">病害列表</span>
                                                        <span class="caption-helper hide">weekly stats...</span>
                                                    </div>
                                                    <div class="actions">
                                                        <a href="javascript:" class="btn btn-primary btn-circle" data-role="add"><i class="fa fa-plus"> 添加病害</i></a>
                                                    </div>
                                                </div>
                                                            ...
                                                            <?php if (!empty($result->list)):?>
                                                                <?php foreach ($result->list as $item):
                                                                    ...
                                                                    <tr data-item='<?=html_attr_json($item)?>'>
                                                                        <td><?=$item->id ?></td>
                                                                        <td><a href="/farming/pool/index"><?=$pools->{$item->pool_id}->name?></a> </td>
                                                                        <td><a href="/farming/batch/index"><?=$item->batch_sn?></a>
...
<script type="text/html" id="tpl-form">
    <form class="form form-horizontal" method="post">
        ...
        <div class="form-group">
            <label class="control-label col-md-3">病害说明 <span class="required">*</span></label>
            <div class="col-md-8">
    ...
    $(function () {
        ...
        $('[data-role=delete]').on('click', function () {
            ...
            layer.confirm('您确定要删除这条信息吗？', function () {
                $.ajax({
                    url: '/farming/ajax/disease/delete',
                    type: 'post',
                    data: {id: id},
        ...
        $modal.find('.ok').on('click', function () {
            ...
            $.ajax({
                url: '/farming/ajax/disease/' + (id ? 'update' : 'create'),
