...
<html>
    ...
    <head>
        <meta charset="utf-8" />
        <title>设备维护记录 - <?=$_TPL['site_name']?></title>
    ...
    <body class="page-header-menu-fixed page-sidebar-closed-hide-logo page-container-bg-solid">
                                                    ...
                                                    <div class="portlet light ">
                                                        <div class="portlet-title">
                                                            <div class="caption caption-md">
                                                                ...
                                                                <span class="caption-subject font-green-steel uppercase bold">设备维护记录</span>
                                                            </div>
                                                        </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">编辑设备维护记录</h4>
                    </div>
        ...
        <script type="text/html" id="tpl-form">
            <form class="form form-horizontal" method="post">
                ...
                <div class="form-group">
                    <label class="control-label col-md-3">维护类型 <span class="required">*</span></label>
                    <div class="col-md-8">
                        <select class="form-control" name="type">
                            <?php foreach ($types as $k => $value):?>
                                <option value="<?=$k?>" <% if (item.type==<?=$k?>) { %>selected<% } %>><?=$value?></option>
                            <?php endforeach;?>
                        </select>
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-md-3">维护时间 <span class="required">*</span></label>
                    <div class="col-md-8">
                        <input class="form-control" name="time_maintained" placeholder="日期时间，如:2010-01-01 00:00:00" value="<%=item.time_maintained%>">
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-md-3">维护说明 <span class="required">&nbsp;</span></label>
                    <div class="col-md-8">
        ...
        <script>
            $(function () {
                ...
                $modal.find('.ok').on('click', function () {
                    ...
                    $.ajax({
                        url: '/farming/ajax/device-maintenance/' + (id ? 'update' : 'create'),
                ...
                $('[data-role=delete]').on('click', function () {
                    ...
                    layer.confirm('您确定删除这条信息吗？', function () {
                        $.ajax({
                            url: '/farming/ajax/device-maintenance/delete?id=' + id,
                            success: function (result) {
                                layer.msg(result.message, function () {
                                    window.location.reload();
                                });
