...
class CultureArea extends Auth
{
    ...
    public function index()
    {
		...
		$response = $this->api->call('producer/breeding-area/get-sum', $params);
        ...
        $this->view->render('statistics/saltwaterfish/culture-area/index');
    ...
    public function push(){
        ...
        $objPHPExcel->getActiveSheet()->MergeCells("B1:B2"); //合并单元格
        $objPHPExcel->getActiveSheet()->MergeCells("C1:C2"); //合并单元格
        $objPHPExcel->getActiveSheet()->MergeCells("D1:E1"); //合并单元格
 ...
 */
            ...
            //先设置标题行
             ...
             ->setCellValue('C1', '网箱养殖（平方米/立方米）')
             ->setCellValue('D1', '工厂化养殖（平方米/立方米）')
             ->setCellValue('D2', '流水养殖')
             ->setCellValue('E2', '循环水养殖')
             
            
             //填充数据/            
             ->setCellValue('A'.$num, $this->quarter[$v->quarter])
             ->setCellValue('B'.$num, $v->池塘养殖)
             ->setCellValue('C'.$num, $v->网箱养殖)
             ->setCellValue('D'.$num, $v->流水养殖)
             ->setCellValue('E'.$num, $v->循环水养殖);
        }
        
        $objPHPExcel->getActiveSheet()->setTitle('海水养殖面积');
        $objPHPExcel->setActiveSheetIndex(0);
        ob_end_clean();
        header('Content-Type: application/vnd.ms-excel');
        header('Content-Disposition: attachment;filename="'.'海水养殖面积统计表'.'.xls"');
