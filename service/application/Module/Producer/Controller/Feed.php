...
use Module\Producer\Model\FeedModel;
...
class Feed extends Api
{
    ...
    public function update()
    {
        ...
        try {
            $result = $feedModel->update($data, ['id=?', [$data['id']]]);
            return $this->export([
                'data' => [
                    'result' => $result
                ]
    ...
    {
        $this->preProcessData($data, FeedModel::$fields);
        ...
        if ($action === 'update') {
            $fields[] = [
                ...
                'rules' => [
                    ...
                    'exists' => function ($field) use ($feedModel) {
                        $item = $feedModel->getItem([
                            'fields' => '1',
                            'id' => $field['value']
                        ]);
                        return [
    ...
    public function getList()
    {
        ...
        $result = $feedModel->getList($params);
        return $this->export([
            'data' => $result
        ]);
    }
