 ...
 */
class ProductionEnvModel extends BaseModel
{
    protected $table = '{production_env}';
    public static $fields = [
        'id', 'pool_id', 'retail_price', 'trade_price', 'time_created', 'time_updated'
    ];
   ...
   public function getListLatest($params = [])
    {
        ...
        $sql = "SELECT A.pool_id, C.name, A.retail_price,  DATE_FORMAT(A.time_created, '%Y年%m月') time_created FROM production_env A"
            . " CROSS JOIN ("
            . " SELECT MAX(time_created) as time_created, pool_id FROM production_env"
            . " GROUP BY pool_id  LIMIT 0,6"
            . " ) B ON A.time_created = B.time_created AND A.pool_id = B.pool_id"
            . " INNER JOIN product_type_category C ON C.id = A.POOL_ID ";
